...
if(changePw != null){
    changePw.addEventListener("submit", e =>{
        ...
        else if( newPw.value.trim().length == 0) str = "새비밀번호를 입력해주세요";
        else if( newPwConfirm.value.trim().length == 0) str = "새 비밀번호를 확인을 입력해주세요";
...
if(secession != null){
    ...
    secession.addEventListener("submit", e =>{
        ...
        // 정말 탈퇴? 물어보기 
        if( !confirm("정말 탈퇴 하시겠습니까?")){// 취소 눌렀을때
            alert("취소되었습니다.");
            e.preventDefault();
            return;
        }
